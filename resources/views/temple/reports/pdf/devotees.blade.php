...
<head>
<meta charset="UTF-8">
<title>Devotees Report — {{ $from }} to {{ $to }}</title>
<style>
...
<div class="summary">
  <div class="stat-box">
    <div class="label">Total Devotees</div>
    <div class="value">{{ $devotees->count() }}</div>
  </div>
  <div class="stat-box">
    <div class="label">Total Receipts</div>
    <div class="value">{{ $devotees->sum('receipts_count') }}</div>
  </div>
  <div class="stat-box">
    <div class="label">Total Contribution</div>
    <div class="value">₹{{ number_format($devotees->sum('total_amount'), 2) }}</div>
  </div>
</div>
...
<table>
  ...
  <tbody>
    @forelse($devotees as $i => $d)
      <tr>
        <td>{{ $i + 1 }}</td>
        <td>{{ $d->name }}</td>
        <td>{{ $d->phone ?? '—' }}</td>
        <td>{{ $d->email ?? '—' }}</td>
        <td style="text-align:right">{{ $d->receipts_count }}</td>
        <td style="text-align:right">{{ number_format($d->total_amount ?? 0, 2) }}</td>
      </tr>
    @empty
      <tr><td colspan="6" style="text-align:center; padding:20px; color:#888;">No devotees found.</td></tr>
    @endforelse
  </tbody>
  <tfoot>
    <tr>
      <td colspan="4" style="text-align:right">Total</td>
      <td style="text-align:right">{{ $devotees->sum('receipts_count') }}</td>
      <td style="text-align:right">₹{{ number_format($devotees->sum('total_amount'), 2) }}</td>
    </tr>
  </tfoot>
</table>
